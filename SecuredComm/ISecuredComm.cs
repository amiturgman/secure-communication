...
interface ISecuredComm
{
    ...
    Task SendEncryptedMsgAsync(string encryptionKeyName, string queueName, Message msg);
    ...
    /// <param name="queueName">Queue name.</param>
    /// <param name="msg">Message.</param>
    Task SendUnencryptedMsgAsync(string queueName, Message msg);
    ...
    string ListenOnEncryptedQueue(string decryptionKeyName, string queueName, Action<Message> cb);

    /// <summary>
    /// Creates a listener on a queue where messages are unencrypted
    /// </summary>
    /// <returns>The consumer tag</returns>
    string ListenOnUnencryptedQueue(string queueName, Action<Message> cb);
